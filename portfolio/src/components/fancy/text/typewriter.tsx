...
export interface TypewriterLinkPart {
  text: string
  href?: string
}
...
interface TypewriterProps {
  /**
   * Text or array of texts to type out. Ignored if `parts` is provided.
   */
  text?: string | string[]

  /**
   * Segments to type out as a single continuous animation, where any segment
   * with an `href` renders as a real (accessible) link once revealed.
   */
  parts?: TypewriterLinkPart[]
...
const Typewriter = ({
  ...
  const renderDisplayText = () => {
    ...
    return parts.map((part, i) => {
      ...
      const segment = displayText.slice(start, offset)
      ...
      return (
        <a
          ...
          className="underline"
        >
          <span className="sr-only">{part.text}</span>
